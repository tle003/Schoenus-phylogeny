...
results_DEC <- readRDS("BioGeoBEARS-0.5/results_DEC_constrained-constrained05-Jan2021.rds")
tr <- read.tree("BioGeoBEARS-0.5/Schoeneae_tree_ultrametric-Jan2021.tre")
tipranges <- getranges_from_LagrangePHYLIP("BioGeoBEARS-0.5/Schoeneae-DEC-9areas.txt")
...
MLstates %>%
  as_tibble(rownames = "node") %>%
  rename(state = value) %>%
  write_csv("BioGeoBEARS-0.5/MLstates.csv")

pdf("BioGeoBEARS-0.5/Schoeneae-constrained_phytools.pdf", width = 10, height = 15)
plotTree(
