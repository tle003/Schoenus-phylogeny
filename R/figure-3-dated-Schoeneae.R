...
write.beast(
  Schoeneae_tree2,
  "data/phylogenies/Cyperaceae-all-taxa-6-calib-st6_Schoeneae.tre"
)
...
my_geoscalePhylo <- function(tree, ages, direction = "rightwards",
                             units = c("Period", "Epoch", "Age"),
                             boxes = "Age", tick.scale = "myr",
                             user.scale,
                             cex.age = 0.3, cex.ts = 0.3, cex.tip = 0.3,
                             width = 1,
                             label.offset,
                             ts.col = TRUE,
                             vers = "ICS2013",
  ...
  if (all(direction != c("rightwards", "upwards"))) {
    return(cat(
      "direction must be either 'rightwards' or 'upwards', here set to 'rightwards'."
    ))
  }
  ...
  units <- rownames(tscale.data)[
    sort(match(units, rownames(tscale.data)), decreasing = T)
  ]
  if (!missing(x.lim)) {
  ...
  timescale.rescaled[, c("Start", "End", "Midpoint")] <-
    root.age -
  ...
  if (tick.scale != "n" |
    ...
    if (tick.scale != "myr" &
        is.numeric(tick.scale) == FALSE) {
      age.name <- subset(
        timescale,
        timescale[, "Type"] == tick.scale &
          timescale[, "Source"] == "ICS"
      )
      age.name <- sort(unique(c(
        age.name[, "Start"],
        age.name[, "End"]
      )))
      age.position <-
  ...
  if (direction == "upwards") {
    ...
    timescale.rescaled.names[
      timescale.rescaled.names[, "Start"] < par()$usr[3],
      "Start"
    ] <- par()$usr[3]
    if (min(timescale.rescaled.names[, "End"]) < par()$usr[4]) {
      ...
      timescale.rescaled.names[
        timescale.rescaled.names[, "End"] > par()$usr[4],
        "End"
      ] <- par()$usr[4]
    }
    ...
    unit.depths <- c(
      par()$usr[2],
      par()$usr[2] - (unit.depths * (par()$usr[2] - par()$usr[1]))
    )
    depth <-
    ...
    if (tick.scale != "n" &&
      ...
      segments(
        (unit.depths[length(unit.depths) - 1]),            tick.position,
        (unit.depths[length(unit.depths)] + depth * 0.75), tick.position,
        lwd = lwd,
        col = col
      )
    }
    for (t in 1:length(units)) {
      ...
      text(
        ...
        cex = cex.ts * tscale.data[
          match(units[t], rownames(tscale.data)),
          "size"
        ],
        srt = tscale.data[
          match(units[t], rownames(tscale.data)),
    ...
    if (!missing(boxes) &&
      ...
      rect(
        par()$usr[3], tscale[, "Start"],
        par()$usr[4], tscale[, "End"],
        col = c("grey90", "white"),
        border = NA
      )
    }
    ...
    if (ranges == TRUE) {
      par(lend = 1)
      segments(
        lastPP$xx[c(1:length(tree$tip.label))], taxon.ranges[, "FAD"],
        lastPP$xx[c(1:length(tree$tip.label))], taxon.ranges[, "LAD"],
        col = "black",
        lwd = width * 2
      )
    }
    if (units[1] == "User") {
      segments(
        par()$usr[1], min(root.age - user.scale[, "Start"]),
        par()$usr[1], max(root.age - user.scale[, "End"])
      )
    } else {
      segments(
        par()$usr[1],
        min(timescale.rescaled[
          timescale.rescaled[, "Type"] == units[1],
          "Start"
        ]),
        par()$usr[1],
    ...
    timescale.rescaled.names[
      timescale.rescaled.names[, "Start"] < par()$usr[1], "Start"
    ] <- par()$usr[1]
    if (min(timescale.rescaled.names[, "End"]) < par()$usr[2]) {
      timescale.rescaled.names <- timescale.rescaled.names[
        timescale.rescaled.names[, "Start"] < par()$usr[2],
      ]
      timescale.rescaled.names[
        timescale.rescaled.names[, "End"] > par()$usr[2], "End"
      ] <- par()$usr[2]
    }
    timescale.rescaled.names[, "Midpoint"] <- (
      timescale.rescaled.names[, "Start"] +
    ...
    if (tick.scale != "n" &&
      ...
      segments(
        tick.position, (unit.depths[length(unit.depths) - 1]),
        tick.position, (unit.depths[length(unit.depths)] + depth * 0.6),
        lwd = lwd,
        col = col
      )
    }
    for (t in 1:length(units)) {
      ...
      if (ts.col == TRUE & units[t] != "User") {
        rect(
          tscale[, "Start"], unit.depths[t],
          tscale[, "End"],   unit.depths[t + 1],
          col = rgb(
            tscale[, "Col_R"],
            tscale[, "Col_G"],
            tscale[, "Col_B"],
    ...
    if (!missing(boxes) &&
      ...
      rect(
        tscale[, "Start"], par()$usr[3],
        tscale[, "End"],   par()$usr[4],
        col = c("grey90", "white"),
        border = NA
      )
    }
    ...
    if (ranges == TRUE) {
      par(lend = 1)
      segments(
        taxon.ranges[, "FAD"], lastPP$yy[c(1:length(tree$tip.label))],
    ...
    if (units[1] == "User") {
      segments(
        min(root.age - user.scale[, "Start"]), par()$usr[3],
