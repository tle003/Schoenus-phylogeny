...
Schoeneae_tree@data <- Schoeneae_tree@data %>%
  mutate(
    height          = height + 100,
    height_median   = height_median + 100,
    height_0.95_HPD = purrr::map(height_0.95_HPD, ~. + 100)
  )
#Schoeneae_tree@phylo$edge.length <- Schoeneae_tree@phylo$edge.length + 100

tree_height <- tree_height + 100
